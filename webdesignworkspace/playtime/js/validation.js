var joinValidate = {
	...
	resultCode : {
		...
		stream_pw : {
			code: 4,
			desc: '같은 문자를 4번이상 사용하실 수 없습니다'
		},
		hangle_pw : {
			code: 5,
			desc: '비밀번호에 한글을 사용하실 수 없습니다'
		},
		other_pw : {
			code: 6,
	...
	// 정규식이
		...
		if(id == ''|| id.length == 0 ){ 							// 1.값이 있는지 없는지 체크
			...
			} else if(id.length < 5 || id.length > 20) {			//6길이(5~20자이내)	
		...
		checkPw : function(pw,rpw) {
			...
			if(pw == '' || pw.length == 0){ 	// 1. 값이 있는지 체크
			...
			} else if (/(\w)\1\1\1/.test(pw)) {  // 4. 같은 값이 4번 연속으로 들어오는지 체크
				...
				} else { 					 // 비밀번호 재확인 값이 같을때 통과 / rpw의 값이 잇을때 통과
	...
	checkEmail : function(email) {
		var regEmpty = /\s/g; // 공백문자
		var regEmail = /^[-A-Za-z0-9]+[-A-Za-z0-9_.]*[@]{1}[-A-Za-z0-9]+[-A-Za-z0-9_.]*[.]{1}[A-Za-z]{1,5}$/g;

		if(email == '' || email.length == 0) { // 값 유무
			return this.resultCode.empty_val;
